...
namespace Mentula.SurvivalGameServer.Commands
{
    public class Teleport : Command
    {
        ...
        {
            if (args.Length < 3) NIMT.ErrorMessage.WriteLine("The Teleport command requires a player name and a total position (Teleport bob 12 45)", null);
            else
            {
                float x;
                float y;

                if (!(float.TryParse(args[1], out x) & float.TryParse(args[2], out y))) NIMT.ErrorMessage.WriteLine("The Teleport command required two floats as arguments!");
                else
                {
                    ...
                    MentulaExtensions.WriteLine(result ? NIMT.Data : NIMT.ErrorMessage, "{0} player: {1}", result ? "Teleported" : "Failed to teleport player", result ? string.Format("{0} to: {1}", args[0], pos) : args[0]);
                }
            }
        }
    }
